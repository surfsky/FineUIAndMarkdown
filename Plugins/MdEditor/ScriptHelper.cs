...
namespace Kingsoc.Web
{
    public class ScriptHelper
    {
        ...
        public static void Alert(string text)
        {
            ...
            string str = string.Format("window.alert('{0}');", text);
        ...
        {
            text = ToScriptSafeString(text);
            string str = string.Format("return window.confirm('{0}');", text);
        ...
        {
            text = ToScriptSafeString(text);
            string str = string.Format("return window.prompt('{0}', '');", text);
            Page.ClientScript.RegisterStartupScript(typeof(string), "Confirm", str, true);
        }
        ...
        {
            string str = string.Format("window.showModalDialog('{0}', window, 'dialogWidth:{1}px;dialogHeight:{2}px;center:Yes;help:No;resizable:Yes;status:No;scroll:{3}');",
                url,
                width,
                height,
                showScroll ? "Yes" : "No"
        ...
        {
            string str = string.Format("window.showModelessDialog('{0}', window, 'dialogWidth:{1}px;dialogHeight:{2}px;center:Yes;help:No;resizable:Yes;status:No;scroll:{3}');",
                url,
                width,
                height,
                showScroll ? "Yes" : "No"
        ...
        public void XmlReLoad()
        {
            ...
            if (!Page.ClientScript.IsStartupScriptRegistered("xmlreload"))
                Page.ClientScript.RegisterStartupScript(typeof(string), "alert", htmlstr.ToString());
        }
